...
void book_push(Vector7d &x_start, Vector7d &x_goal, double &goal_thr,
               ...
               Vector3d &x_ub, Vector3d &x_lb, PlanningWorld *pw, std::vector<ContactPoint> *surface)
{
    ...
    SkeletonPtr block =
        createFixedBox("block", Vector3d(5*box_width, 5*box_width, 5*box_width),
                       Vector3d(5, 5, box_width / 2));

    // SkeletonPtr block =
    //     createFixedBox("block", Vector3d(box_width, box_width, box_width),
    //                    Vector3d(2, 2, box_width / 2));
    // SkeletonPtr book2 =
    ...
    SkeletonPtr ground =
        createFixedBox("ground", Vector3d(50, 50, 1), Vector3d(0, 0, -0.5));

    my_world->addObject(object);
    my_world->addEnvironmentComponent(block);
    ...
    DartPointManipulator *rpt =
        new DartPointManipulator(n_robot_contacts, 1/8 * box_width);
    ...
    // x_goal << 1, 1, box_height/2, 0, 0, 0, 1;
    x_goal << 12, 12, box_height/2, 0, 0, 0.4999145, 0.8660748;
    x_ub << 30, 30, box_length * 3;
    x_lb << -10, -10, 0.0;
...
int main(int argc, char *argv[])
{
    ...
    book_push(x_start, x_goal, goal_thr, wa, wt, eps_trans, eps_angle, x_ub, x_lb, &pw, &surface);
