              ...
              Vector3d &x_ub, Vector3d &x_lb, PlanningWorld *pw, std::vector<ContactPoint> *surface)
{
    ...
    DartPointManipulator *rpt =
        new DartPointManipulator(n_robot_contacts, 0.001+0.0005);
...
int main(int argc, char *argv[])
{
    ...
    tabletop(obj_body_id, robot_body_ids, env_body_ids, obj_body_qadr, obj_body_dofadr,
             goal_thr, wa, wt, eps_trans, eps_angle, x_ub, x_lb,
             &pw, &surface);
    ...
    double dt = 0.1;//max_time / mnp_poses.size();
    ...
    // TODO: handle geom_pos. This is a mismatch of mujoco and planner
    // We can assume geom_pos = 0 for ease
    double obj_start_pos_x = m->body_pos[obj_body_id*3+0];// + m->geom_pos[obj_geomadr*3+0];
    double obj_start_pos_y = m->body_pos[obj_body_id*3+1];// + m->geom_pos[obj_geomadr*3+1];
    double obj_start_pos_z = m->body_pos[obj_body_id*3+2];// + m->geom_pos[obj_geomadr*3+2];

    double obj_start_quat_w = m->body_quat[obj_body_id*4+0];
    double obj_start_quat_x = m->body_quat[obj_body_id*4+1];
    double obj_start_quat_y = m->body_quat[obj_body_id*4+2];
    double obj_start_quat_z = m->body_quat[obj_body_id*4+3];

    x_goal << obj_start_pos_x - 0.04, obj_start_pos_y + 0.05, obj_start_pos_z, 
              obj_start_quat_x, obj_start_quat_y, obj_start_quat_z, obj_start_quat_w;

    bool REPLAN = true;
    // reset x_start
    while (REPLAN)
    {
        Vector7d x_start;  // start from current object position

        double obj_pos_x = d->qpos[obj_body_qadr+0];
        double obj_pos_y = d->qpos[obj_body_qadr+1];
        double obj_pos_z = d->qpos[obj_body_qadr+2];

        double obj_quat_w = d->qpos[obj_body_qadr+3];
        double obj_quat_x = d->qpos[obj_body_qadr+4];
        double obj_quat_y = d->qpos[obj_body_qadr+5];
        double obj_quat_z = d->qpos[obj_body_qadr+6];
        ...
        std::cout << "after visualize path " << std::endl;
        //
        ...
        for (auto &k : node_path) {
            ...
            // sometimes the first element in mnp_configs is empty, causing segmentation fault
            }
        }

        std::cout << "after setting mnp_configs and object_poses... " << std::endl;
        std::cout << "mnp_configs size: " << mnp_configs.size() << std::endl;

        std::cout << "object_poses size: " << object_poses.size() << std::endl;

        std::vector<std::vector<Vector3d>> mnp_poses;
        std::vector<std::vector<Vector3d>> mnp_poses_in_obj;
        ...
        for (int i=0; i<mnp_configs.size(); i++)
        {
            ...
            std::cout << "object poses: " << pose.transpose() << std::endl;
            ...
            std::cout << "mnp_configs[" << i << "]: " << mnp_configs[i].transpose() << std::endl;
            // each robot point: get the pose of the point
            std::vector<Vector3d> mnp_poses_i;
            std::vector<Vector3d> mnp_poses_in_obj_i;
            for (int j=0; j<robot_body_ids.size(); j++)
            {
                mnp_poses_i.push_back(T.block(0,0,3,3)*mnp_configs[i].segment<3>(j*6) + T.block(0,3,3,1));
                mnp_poses_in_obj_i.push_back(mnp_configs[i].segment<3>(j*6));
            }
            mnp_poses.push_back(mnp_poses_i);
            mnp_poses_in_obj.push_back(mnp_poses_in_obj_i);
        }
        std::cout << "after setting mnp_poses..." << std::endl;
        ...
        std::vector<std::vector<toppra::Vectors>> toppra_positions;  // size: N points x N segments x N times
        // toppra::Vectors toppra_positions;

        for (int i=0; i<mnp_poses[0].size(); i++)
        {
            std::vector<toppra::Vectors> toppra_positions_i;
            int idx = 0;
            while (idx < mnp_poses.size())
            {
                ...
                {
                    // compare with previous positiion
                    Vector3d diff =  mnp_poses_in_obj[idx][i] - mnp_poses_in_obj[idx-1][i];
                    if (diff.norm() <= 1e-3)
                    {
                        // fixed pose
                        CREATE_NEW = false;
                ...
                if (CREATE_NEW)
                {
                    ...
                    toppra::Vectors toppra_positions_i_j;
                    toppra_positions_i.push_back(toppra_positions_i_j);
                }
                toppra::Vector position(mnp_poses[idx][i].size());
                for (int k=0; k<mnp_poses[idx][i].size(); k++)
                {
                    // add to the last segment
                    position(k) = mnp_poses[idx][i][k];
                }
                toppra_positions_i.back().push_back(position);
                idx += 1;
            }
            toppra_positions.push_back(toppra_positions_i);
        }
        std::cout << "toppra_positions.size(): " << toppra_positions.size() << std::endl;
        // testing: in python
        // toppra_positions: N robots x N segments x N_k times
        ...
        std::vector<std::vector<toppra::PiecewisePolyPath>> splines;  // N robots x N segments
        std::vector<std::vector<double>> time_end;
        std::vector<std::vector<double>> time_start;
        
        // std::vector<toppra::PiecewisePolyPath> splines;  // N points
        for (int i=0; i<toppra_positions.size(); i++)
        {
            // for each robot
            int times_s_idx = 0;
            std::vector<toppra::PiecewisePolyPath> splines_i;
            std::vector<double> time_end_i;
            std::vector<double> time_start_i;
            for (int j=0; j<toppra_positions[i].size(); j++)
            {
                std::cout << "toppra_positions[i][j].size: " << toppra_positions[i][j].size() << std::endl;
                std::cout << "times.segments: " << times.segment(times_s_idx,toppra_positions[i][j].size()).transpose() << std::endl;
                // segment
                toppra::PiecewisePolyPath spline = 
                        toppra::PiecewisePolyPath::CubicSpline(toppra_positions[i][j], 
                                                               times.segment(times_s_idx,toppra_positions[i][j].size()), 
                                                               bc_type);
                time_start_i.push_back(times(times_s_idx));
                times_s_idx += toppra_positions[i][j].size();
                splines_i.push_back(spline);
                time_end_i.push_back(times(times_s_idx-1));
                std::cout << "time_end_i: " << time_end_i.back() << std::endl;

            }

            splines.push_back(splines_i);
            time_end.push_back(time_end_i);
            time_start.push_back(time_start_i);
        }
        ...
        for (int i=0; i<robot_body_jntadrs.size(); i++)
        {
            ...
            d->ctrl[robot_p_actuator_ids[i][0]] = mnp_poses[traj_idx][i][0];
            d->ctrl[robot_p_actuator_ids[i][1]] = mnp_poses[traj_idx][i][1];
            d->ctrl[robot_p_actuator_ids[i][2]] = mnp_poses[traj_idx][i][2];

            d->ctrl[robot_v_actuator_ids[i][0]] = 0;
            d->ctrl[robot_v_actuator_ids[i][1]] = 0;
            d->ctrl[robot_v_actuator_ids[i][2]] = 0;
        }
        mj_forward(m, d);
        // mj_step(m, d);
        ...
        for (int i=0; i<robot_body_jntadrs.size(); i++)
        {
            segment_inds.push_back(0);
        }

        /* visualize the trajectory */
        while (!glfwWindowShouldClose(window))
        {
            if (d->time - total_simstart < dt*(mnp_poses.size()-1))
            {
                ...
                // for each robot pt, find the segment of the time


                for (int i=0; i<splines.size(); i++)
                {
                    while (d->time - total_simstart > time_end[i][segment_inds[i]])
                    {
                        std::cout << "d->time - total_simstart: " << d->time - total_simstart << std::endl;
                        std::cout << "segment_inds[" << i << "]:" << segment_inds[i] << std::endl;
                        std::cout << "time_end size: " << time_end[i].size() << std::endl;
                        std::cout << "time_end: " << time_end[i][segment_inds[i]] << std::endl;

                        segment_inds[i] += 1;

                        // teleport the point
                        Vector3d pos = splines[i][segment_inds[i]].eval_single(time_start[i][segment_inds[i]],0);

                        d->qpos[m->jnt_qposadr[robot_body_jntadrs[i]+0]] = pos[0];
                        d->qpos[m->jnt_qposadr[robot_body_jntadrs[i]+1]] = pos[1];
                        d->qpos[m->jnt_qposadr[robot_body_jntadrs[i]+2]] = pos[2];
                    ...
                    // if the time is not yet time_start_i, then do nothing
                    if (d->time - total_simstart > time_start[i][segment_inds[i]])
                    {
                        Vector3d pos = splines[i][segment_inds[i]].eval_single(d->time - total_simstart,0);
                        Vector3d vel = splines[i][segment_inds[i]].eval_single(d->time - total_simstart,1);
                        ...
                        //if too faraway from the desired position, jump there
                ...
                // when desired pose deviate too much from the actual pose, replan
                // stored quat: qx,qy,qz,qw
                // mujoco: qw,qx,qy,qz

                double pos_threshold = 0.02;
                double ang_threshold = 5*M_PI/180;

                Vector7d desired_pose = object_poses[traj_idx];  // qx,qy,qz,qw
                // qx,qy,qz,qw -> qw,qx,qy,qz
                desired_pose[3] = object_poses[traj_idx][6];
                desired_pose[4] = object_poses[traj_idx][3];
                desired_pose[5] = object_poses[traj_idx][4];
                desired_pose[6] = object_poses[traj_idx][5];
                Matrix4d desired_T = pose2SE3(desired_pose.data());  // qw,qx,qy,qz

                Vector7d actual_pose;  // qx,qy,qz,qw
                actual_pose << d->qpos[obj_body_qadr+0],
                                d->qpos[obj_body_qadr+1],
                                d->qpos[obj_body_qadr+2],
                                d->qpos[obj_body_qadr+3],
                                d->qpos[obj_body_qadr+4],
                                d->qpos[obj_body_qadr+5],
                                d->qpos[obj_body_qadr+6];

                Matrix4d actual_T = pose2SE3(actual_pose.data());  // qw,qx,qy,qz

                Matrix4d delta_T = desired_T * actual_T.inverse();


                Vector6d T_se3 = SE32se3(delta_T);
                double ang_diff = T_se3.tail(3).norm();
                Vector3d pos_diff_vec = desired_pose.head(3) - actual_pose.head(3);
                double pos_diff = pos_diff_vec.norm();

                std::cout << "position diff: " << pos_diff << "/" << pos_threshold << std::endl;
                std::cout << "angle diff (degrees): " << ang_diff / M_PI * 180 << "/" << 5 << std::endl;

                if ((pos_diff > pos_threshold) || (ang_diff > ang_threshold))
                {
                    REPLAN = true;
                    break;
                }
            ...
            else
            {
                ...
                Vector7d desired_pose = x_goal;  // qx,qy,qz,qw
                // qx,qy,qz,qw -> qw,qx,qy,qz
                desired_pose[3] = x_goal[6];
                desired_pose[4] = x_goal[3];
                desired_pose[5] = x_goal[4];
                desired_pose[6] = x_goal[5];
                Matrix4d desired_T = pose2SE3(desired_pose.data());  // qw,qx,qy,qz
                ...
                std::cout << "final position diff: " << pos_diff << "/" << pos_threshold << std::endl;
                std::cout << "final angle diff (degrees): " << ang_diff / M_PI * 180 << "/" << 5 << std::endl;
